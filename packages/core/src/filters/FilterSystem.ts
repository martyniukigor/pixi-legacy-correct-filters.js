...
export class FilterSystem implements ISystem
{
    ...
    push(target: IFilterTarget, filters: Array<Filter>): void
    {
        ...
        let autoFitY = filters[0].autoFitY;
        let autoFitWidth = filters[0].autoFitWidth;
        let autoFitHeight = filters[0].autoFitHeight;
        ...
        state.renderTexture = this.getOptimalFilterTexture(autoFitWidth ? autoFitWidth : state.sourceFrame.width,
            autoFitHeight ? autoFitHeight : state.sourceFrame.height,
            resolution, multisample);
        state.filters = filters;

        state.destinationFrame.width = autoFitWidth ? autoFitWidth : state.renderTexture.width;
        state.destinationFrame.height = autoFitHeight ? autoFitHeight : state.renderTexture.height;

        const destinationFrame = this.tempRect;

        destinationFrame.x = 0;
        destinationFrame.y = 0;
        destinationFrame.width = autoFitWidth ? autoFitWidth : state.sourceFrame.width;
        destinationFrame.height = autoFitHeight ? autoFitHeight : state.sourceFrame.height;
